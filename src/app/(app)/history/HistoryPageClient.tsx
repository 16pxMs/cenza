...
function recapDisplayLabel(label: string) {
  if (label === 'Fixed' || label === 'Fixed spending') return 'Fixed'
...
export default function HistoryPageClient({ data, targetCycleId, currentCycleId }: HistoryPageClientProps) {
  ...
  const segments = [
    { label: 'Fixed', amount: data.breakdown.find(i => i.label === 'Fixed')?.amount ?? 0, color: 'var(--category-essentials)' },
        ...
        {data.rows.length === 0 && (
          <div style={{
            ...
            <p style={{
              ...
              Once you log expenses, this page will show where your money went across Fixed, Spending, and Debt.
            </p>
            ...
            <div style={{
              ...
              {['Fixed', 'Spending', 'Debt'].map((label, index) => (
                <div
                  key={label}
                  style={{
                    display: 'flex',
