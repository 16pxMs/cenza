...
async function loadTransactionCategoryRows() {
  ...
  while (true) {
    ...
    if (error) {
      throw new Error(`Failed to read transactions for migration planning: ${error.message}`)
    }
...
async function main() {
  ...
  const audit = buildCategoryAuditReport(rows)
  const plan = buildCategoryMigrationPlan(rows)

  const output = {
    generatedAt: new Date().toISOString(),
    totalRows: plan.totalRows,
    summary: plan.summary,
    mappingTable: plan.mappingTable,
    examplesByBucket: plan.examplesByBucket,
    preview: plan.preview,
    unknownCount: plan.unknownCount,
    auditProblemSummary: audit.problemRows.reduce((summary, row) => {
...
main().catch((error) => {
  console.error('[plan-category-migration] failed')
