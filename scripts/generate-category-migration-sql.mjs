...
async function main() {
  ...
  const sqlPlan = buildCategoryMigrationSqlPlan(rows)

  const output = {
    summary: sqlPlan.summary,
    previewQuery: sqlPlan.previewQuery,
    updateQueries: sqlPlan.updateQueries,
    transactionSql: sqlPlan.transactionSql,
  }
...
main().catch((error) => {
  console.error('[generate-category-migration-sql] failed')
